...
openssl req -passin pass:1111 -new -x509 -days 365 -key ca.key -out ca.crt -subj  "/C=US/ST=KS/L=Lenexa/O=LeggettandPlatt/OU=ConsumerProducts/CN=LenexaR&D"

echo -e "\n${YELLOW}Generate server key${NC}"
openssl genrsa -passout pass:1111 -des3 -out server.key 4096

echo -e "\n${YELLOW}Generate server signing request${NC}"
openssl req -passin pass:1111 -new -key server.key -out server.csr -subj  "/C=US/ST=KS/L=Lenexa/O=LeggettandPlatt/OU=ConsumerProducts/CN=bigdata"
...
openssl req -passin pass:1111 -new -key client.key -out client.csr -subj  "/C=US/ST=KS/L=Lenexa/O=LeggettandPlatt/OU=ConsumerProducts/CN=k8s"
